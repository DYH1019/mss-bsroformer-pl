...
from separate import sep_weighted_avg, sep_weighted_avg_shift
...
def run_folder(model, args, config, device, verbose=False):
    ...
    for path in all_mixtures_path:
        ...
        mix_orig = mix.copy()
        mix_orig = torch.from_numpy(mix_orig).to(device)
        ...
        seg_len = config.inference.seg_len
        ...
        for mix in track_proc_list:
            ...
            mix = torch.unsqueeze(mix, axis=0)
            # print(mix.dtype)
            # print(mix.shape)
            waveforms = sep_weighted_avg_shift(model, mix, seg_len, config)
        ...
        for instr in instruments:
            ...
            # else:
            output_file = os.path.join(args.store_dir, f"{file_name}_{instr}_zd.wav")
                
            estimates = estimates.T.squeeze(0).T.cpu().numpy()
...
def proc_folder(args):
    ...
    ckpt_path = Path(args.start_check_point) if args.start_check_point is not None else None
    ...
    if args.start_check_point != '':
        ...
        if ckpt_path.suffix == '.ckpt':
            state_dict = load_pl_state_dict(args.start_check_point, device='cpu')
        else:
